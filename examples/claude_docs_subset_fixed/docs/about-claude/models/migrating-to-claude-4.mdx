---
title: "Before (Claude Sonnet 4)"
description: "# Models overview"
---

This guide covers two key migration paths to Claude 4.5 models:

* **Claude Sonnet 3.7 → Claude Claude Sonnet 4.5**: Our most intelligent model with best-in-class reasoning, coding, and long-running agent capabilities
* **Claude Haiku 3.5 → Claude Claude Haiku 4.5**: Our fastest and most intelligent Haiku model with near-frontier performance for real-time applications and high-volume intelligent processing
...
Before starting your migration, we recommend reviewing [What's new in Claude 4.5](/docs/about-Claude/models/whats-new-Claude-4-5) to understand the new features and capabilities available in these models, including Extended Thinking, context awareness, and behavioral improvements.

## Migrating from Claude Sonnet 3.7 to Claude Claude Sonnet 4.5

Claude Claude Sonnet 4.5 is our most intelligent model, offering best-in-class performance for reasoning, coding, and long-running autonomous agents. This migration includes several breaking changes that require updates to your implementation.
...
3. **Handle the new `refusal` stop reason**
   ...
   Update your application to [handle `refusal` stop reasons](/docs/test-and-evaluate/strengthen-guardrails/handle-streaming-refusals):
...
4. **Update text editor tool (if applicable)**
   ...
   See [Text editor tool documentation](/docs/agents-and-tools/tool-use/text-editor-tool) for details.

5. **Update code execution tool (if applicable)**

   Upgrade to `code_execution_20250825`. The legacy version `code_execution_20250522` still works but is not recommended. See [Code execution tool documentation](/docs/agents-and-tools/tool-use/code-execution-tool#upgrade-to-latest-tool-version) for migration instructions.

6. **Remove token-efficient tool use beta header**

   [Token-efficient tool use](/docs/agents-and-tools/tool-use/token-efficient-tool-use) is a beta feature in Claude 3.7. All Claude 4 models have built-in token-efficient tool use, so you should no longer include the beta header.

   Remove the `token-efficient-tools-2025-02-19` [beta header](/api/beta-headers) from your requests:
...
7. **Remove extended output beta header**
   ...
   The `output-128k-2025-02-19` [beta header](/api/beta-headers) for extended output is only available in Claude Sonnet 3.7.
...
8. **Update your prompts for behavioral changes**
   ...
   Claude Claude Sonnet 4.5 has a more concise, direct communication style and requires explicit direction. Review [Claude 4 prompt engineering best practices](/docs/build-with-Claude/prompt-engineering/Claude-4-best-practices) for optimization guidance.

9. **Consider enabling Extended Thinking for complex tasks**

   Enable [Extended Thinking](/docs/build-with-Claude/extended-thinking) for significant performance improvements on coding and reasoning tasks (disabled by default):
   ...
   <Note>
     Extended Thinking impacts [Prompt Caching](/docs/build-with-Claude/prompt-caching#caching-with-thinking-blocks) efficiency.
   </Note>
...
* [ ] Review and update prompts following [Claude 4 best practices](/docs/build-with-Claude/prompt-engineering/Claude-4-best-practices)
* [ ] Consider enabling Extended Thinking for complex reasoning tasks
* [ ] Handle `model_context_window_exceeded` stop reason (Claude Sonnet 4.5 specific)
...
## Migrating from Claude Haiku 3.5 to Claude Claude Haiku 4.5
...
For a complete overview of new capabilities, see [What's new in Claude 4.5](/docs/about-Claude/models/whats-new-Claude-4-5#key-improvements-in-haiku-4-5-over-haiku-3-5).

<Note>
  Claude Haiku 4.5 pricing $1 per million input tokens, $5 per million output tokens. See [Claude pricing](/docs/about-Claude/pricing) for details.
</Note>
...
2. **Update tool versions (if applicable)**
   ...
   Claude Haiku 4.5 only supports the latest tool versions:
...
4. **Review new rate limits**
   ...
   Claude Haiku 4.5 has separate rate limits from Haiku 3.5. See [Rate limits documentation](/api/rate-limits) for details.
...
7. **Explore new capabilities**
   ...
   See [What's new in Claude 4.5](/docs/about-Claude/models/whats-new-Claude-4-5#key-improvements-in-haiku-4-5-over-haiku-3-5) for details on context awareness, increased output capacity (64K tokens), higher intelligence, and improved speed.
...
* [ ] use context awareness for better token management in long sessions
* [ ] Prepare for larger responses (max output increased from 8K to 64K tokens)
* [ ] Review and update prompts following [Claude 4 best practices](/docs/build-with-Claude/prompt-engineering/Claude-4-best-practices)
* [ ] Test in development environment before production deployment

## Choosing between Claude Sonnet 4.5 and Claude Haiku 4.5

Both Claude Claude Sonnet 4.5 and Claude Claude Haiku 4.5 are powerful Claude 4 models with different strengths:

### Choose Claude Claude Sonnet 4.5 (most intelligent) for:
...
### Choose Claude Claude Haiku 4.5 (fastest and most intelligent Haiku) for:
...
Claude 4 models, particularly Sonnet and Claude Haiku 4.5, show significant performance improvements when using [Extended Thinking](/docs/build-with-Claude/extended-thinking) for coding and complex reasoning tasks. Extended Thinking is **disabled by default** but we recommend enabling it for demanding work.

**Important**: Extended Thinking impacts [Prompt Caching](/docs/build-with-Claude/prompt-caching#caching-with-thinking-blocks) efficiency. When non-tool-result content is added to a conversation, thinking blocks are stripped from cache, which can increase costs in multi-turn conversations. We recommend enabling thinking when the performance benefits outweigh the caching trade-off.
...
### Migrating from Claude Sonnet 4 → Claude Sonnet 4.5
...
## Migrating from Claude Claude Opus 4.1 → Claude Sonnet 4.5
...
Claude Claude Sonnet 4.5 is our most intelligent model with best-in-class reasoning, coding, and long-running agent capabilities. It offers superior performance compared to Claude Opus 4.1 for most use cases.

## Need help?

* Check our [API documentation](/api/overview) for detailed specifications
* Review [model capabilities](/docs/about-Claude/models/overview) for performance comparisons
* Review [API release notes](/release-notes/api) for API updates
